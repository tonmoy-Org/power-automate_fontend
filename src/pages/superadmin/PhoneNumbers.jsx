  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ px: 3, py: 2.5 }}>
        <DialogContentText
          sx={{ fontSize: "0.875rem", color: TEXT, lineHeight: 1.6 }}
        >
          {message}
        </DialogContentText>
      </DialogContent>
  ...
  return (
    <Accordion
      sx={{
        ...
        "&:before": { display: "none" },
        overflow: "hidden",
        backgroundColor: alpha(W, 0.02),
      }}
      elevation={0}
    >
      <AccordionSummary
        expandIcon={<ExpandMoreIcon sx={{ color: W }} />}
        sx={{
          backgroundColor: alpha(W, 0.06),
          borderBottom: `1px solid ${alpha(W, 0.2)}`,
          minHeight: 48,
      ...
      >
        <Box display="flex" alignItems="center" gap={1.5}>
          ...
          <Typography sx={{ fontSize: "0.85rem", fontWeight: 600, color: W }}>
            {duplicates.length} Duplicate Number
    ...
    >
      <Box
        ...
        sx={{
          px: 2,
          py: 1.5,
          backgroundColor: alpha(RED, 0.06),
          borderBottom: `1px solid ${alpha(RED, 0.2)}`,
        }}
      >
        <Box display="flex" alignItems="center" gap={1}>
            ...
            sx={{
              ...
              borderBottom:
                index < errors.length - 1
                  ? `1px solid ${alpha(RED, 0.1)}`
                  : "none",
              backgroundColor:
                index % 2 === 0 ? alpha(RED, 0.02) : "transparent",
            }}
          >
...
const CountryCodeRow = memo(({
  ...
  onDeleteGroup,
  theme,
  colors: { BLUE, BLUE_DARK, RED, GREEN, TEXT },
  onSuccess,
  onError,
  globalStartIndex,
  absoluteGroupIdx,
  ...
  const { data: innerNumbers = [], isLoading: isInnerLoading } = useQuery({
    queryKey: ["phoneNumbersGroup", group.country_code],
    queryFn: async () => {
      const response = await axiosInstance.get(`/phone-numbers`, {
        params: {
  ...
  }, [items, statusFilter, debouncedInnerSearch]);
  ...
  const handleInnerMarkStatus = (targetStatus) => {
    const toUpdate = filteredItems.filter(
      (i) => innerSelected.includes(i._id) && i.is_active !== targetStatus,
    );
      ...
      confirmData: {
        ...
        message: (
          <>
            ...
            <strong>
              {count} phone number{count !== 1 ? "s" : ""}
            </strong>{" "}
            as <strong style={{ color: statusColor }}>{statusLabel}</strong>?
          </>
        ),
  ...
  const clearInnerSearch = () => {
    setInnerSearchQuery("");
    setDebouncedInnerSearch("");
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Tooltip
            title="Select group for global bulk operations"
            placement="top"
          >
            <Checkbox
              size="small"
              ...
              onChange={(e) => {
                e.stopPropagation();
                onGlobalSelectGroup(groupIds, globalAllSelected);
              }}
            />
          </Tooltip>
        </TableCell>
          ...
          <Box display="flex" alignItems="center" gap={1} flexWrap="wrap">
            ...
            {statusCounts.inactive > 0 && (
              <Chip
                ...
                label={`${statusCounts.inactive} Inactive`}
                sx={{
                  height: 20,
                  borderRadius: "4px",
                  fontSize: "0.7rem",
                  fontWeight: 600,
                  backgroundColor: alpha(STATUS_COLORS.inactive.color, 0.1),
                  color: STATUS_COLORS.inactive.color,
                  border: `1px solid ${alpha(STATUS_COLORS.inactive.color, 0.3)}`,
                  "& .MuiChip-label": { px: 0.7 },
                  "& .MuiChip-icon": { ml: 0.5 },
                }}
              />
            ...
            {statusCounts.running > 0 && (
              <Chip
                size="small"
                icon={<PlayArrowIcon sx={{ fontSize: "0.65rem !important" }} />}
                ...
                sx={{
                  ...
                  fontSize: "0.7rem",
                  fontWeight: 600,
                  backgroundColor: alpha(STATUS_COLORS.running.color, 0.1),
                  color: STATUS_COLORS.running.color,
                  border: `1px solid ${alpha(STATUS_COLORS.running.color, 0.3)}`,
                  "& .MuiChip-label": { px: 0.7 },
                  "& .MuiChip-icon": { ml: 0.5 },
                }}
              />
            ...
            {statusCounts.completed > 0 && (
              <Chip
                size="small"
                icon={<DoneIcon sx={{ fontSize: "0.65rem !important" }} />}
                ...
                sx={{
                  ...
                  fontSize: "0.7rem",
                  fontWeight: 600,
                  backgroundColor: alpha(STATUS_COLORS.completed.color, 0.1),
                  color: STATUS_COLORS.completed.color,
                  border: `1px solid ${alpha(STATUS_COLORS.completed.color, 0.3)}`,
                  "& .MuiChip-label": { px: 0.7 },
                  "& .MuiChip-icon": { ml: 0.5 },
                }}
              />
            ...
            {hasGlobalChild && (
              <Chip
                size="small"
                label={`${globalSelectedInGroup.length} in bulk`}
                sx={{
                  height: 20,
                  borderRadius: "4px",
                  fontSize: "0.7rem",
        ...
        >
          <Tooltip
            title={`Delete all ${totalGroupCount} numbers in ${group.country_code}`}
            placement="top"
          >
            <IconButton
              size="small"
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{ borderLeft: `3px solid ${alpha(BLUE, 0.2)}`, ml: 3 }}>
              ...
              <Box
                sx={{
                  p: 1.5,
                  borderBottom: `1px solid ${alpha(BLUE, 0.12)}`,
                  backgroundColor: alpha(
                    BLUE,
              ...
              >
                <StyledTextField
                  fullWidth
                  size="small"
                  placeholder="Search by number or RDP ID..."
                  ...
                  InputProps={{
                    startAdornment: (
                      <InputAdornment position="start">
                        <SearchIcon
                          sx={{ fontSize: "0.9rem", color: alpha(TEXT, 0.4) }}
                        />
                      </InputAdornment>
                    ),
                    endAdornment: innerSearchQuery && (
                      <InputAdornment position="end">
                        <IconButton
                          size="small"
                          onClick={clearInnerSearch}
                          sx={{ p: 0.3 }}
                        >
                          <ClearIcon
              ...
              {innerSelected.length > 0 && (
                ...
                >
                  <Typography
                    sx={{ fontSize: "0.78rem", color: BLUE, fontWeight: 600 }}
                  >
                  ...
                  {STATUS_ENUM.map((status) => {
                    const selectedWithDifferentStatus = filteredItems.filter(
                      (i) =>
                        innerSelected.includes(i._id) && i.is_active !== status,
                    ).length;
                    return selectedWithDifferentStatus > 0 ? (
                      <Button
                        ...
                        size="small"
                        variant="contained"
                        disabled={innerConfirmDialog.loading}
                        startIcon={
                          innerConfirmDialog.loading ? (
                            <CircularProgress
                              size={12}
                              sx={{ color: "white" }}
                            />
                        ...
                        sx={{
                          ...
                          height: 26,
                          textTransform: "none",
                          background: `linear-gradient(135deg, ${STATUS_COLORS[status].color} 0%, ${alpha(STATUS_COLORS[status].color, 0.7)} 100%)`,
                          color: "white",
                          borderRadius: "6px",
                          "&:hover": { filter: "brightness(1.1)" },
                        }}
                  ...
                  <Button
                    ...
                    onClick={() => onBulkEdit(innerSelected)}
                    sx={{
                      fontSize: "0.72rem",
                      py: 0.3,
                      px: 1,
                  ...
                  <Button
                    ...
                    startIcon={
                      innerConfirmDialog.loading ? (
                        ...
                        <DeleteSweepIcon sx={{ fontSize: "0.8rem" }} />
                      )
                    }
                    ...
                    sx={{
                      ...
                      height: 26,
                      textTransform: "none",
                      background: `linear-gradient(135deg, ${theme.palette.error.dark} 0%, ${RED} 100%)`,
                      color: "white",
                      borderRadius: "6px",
                      "&:hover": { filter: "brightness(1.1)" },
                    }}
                    ...
                    {[
                      ...
                      <TableCell
                        ...
                        sx={{
                          ...
                          color: TEXT,
                          borderBottom: `1px solid ${alpha(BLUE, 0.18)}`,
                          pl: i === 0 || i === 1 ? 2 : undefined,
                          pr: i === 5 ? 1.5 : undefined,
                        }}
                      >
                        {label}
                      </TableCell>
                    ...
                    pagedItems.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell sx={{ ...cellSx, pl: 2, width: 60 }}>
                            <Typography
                              sx={{
                                fontSize: "0.82rem",
                                fontFamily: "monospace",
                            ...
                            >
                              {innerPage * INNER_PAGE_SIZE + pagedItems.indexOf(item) + 1}
                            </Typography>
                          </TableCell>
              ...
              {filteredItems.length > INNER_PAGE_SIZE && (
                <Box
                  sx={{
                    borderTop: `1px solid ${alpha(BLUE, 0.12)}`,
                    backgroundColor: alpha(
                      BLUE,
                      theme.palette.mode === "dark" ? 0.03 : 0.01,
                    ),
                  }}
                >
                  <TablePagination
...
export const PhoneNumbers = () => {
  ...
  const TEXT = theme.palette.text.primary;
  const colors = { BLUE, BLUE_DARK, RED, GREEN, TEXT };
  ...
  const openConfirm = (cfg) =>
    setConfirmDialog({ ...CONFIRM_DEFAULT, open: true, ...cfg });
  ...
  const {
    ...
    queryKey: ["phoneNumbers", debouncedSearch],
    queryFn: () => {
      const params = {};
      ...
      return fetchPhoneNumbers(params);
    },
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (data) => {
      await queryClient.invalidateQueries(["phoneNumbers"]);
      await queryClient.invalidateQueries(["phoneNumbersGroup"]);
      await refetch();
  ...
  const updateMutation = useMutation({
    ...
    onSettled: () => {
      ...
      queryClient.invalidateQueries(["phoneNumbersGroup"]);
    },
  });
  ...
  const filteredNumbers = useMemo(() => {
    if (statusFilter === "all") return allPhoneNumbers;

    // Summary mode (where array elements are pre-aggregated country_code groups)
    if (allPhoneNumbers.length > 0 && allPhoneNumbers[0]?.count !== undefined) {
      return allPhoneNumbers.filter((group) => {
    ...
    return allPhoneNumbers.filter((item) => item.is_active === statusFilter);
  }, [allPhoneNumbers, statusFilter]);

  const groupedNumbers = useMemo(() => {
    if (filteredNumbers.length > 0 && filteredNumbers[0]?.count !== undefined) {
      return filteredNumbers.map((group) => ({
        country_code: group.country_code,
        items: [group]
      }));
    }
    return groupByCountryCode(filteredNumbers);
  }, [filteredNumbers]);
  ...
  const allVisibleIds = useMemo(() => {
    if (filteredNumbers.length > 0 && filteredNumbers[0]?.ids !== undefined) {
      return filteredNumbers.flatMap((item) => item.ids || []);
    }
    return filteredNumbers.map((item) => item._id);
  }, [filteredNumbers]);
  ...
  const handleGroupSelection = (type, countryCode = null) => {
    ...
    if (type === 'all') {
      ...
      setFormData((prev) => {
        ...
        return { ...prev, password_formatter_ids: nextIds };
      });
    }
  };
  ...
  useEffect(() => {
    if (
      ...
      setFormData((prev) => ({
        ...prev,
        password_formatter_ids: resolved,
  ...
  const handleDeleteGroupClick = useCallback((group) => {
    const totalCount = (group.items.length > 0 && group.items[0]?.count !== undefined)
      ? group.items[0].count
      : group.items.length;
    openConfirm({
      title: "Delete Group",
      ...
      message: (
        <>
          ...
          <strong>
            {totalCount} number{totalCount !== 1 ? "s" : ""}
          </strong>{" "}
          under <strong>{group.country_code}</strong>? This cannot be undone.
        </>
      ),
      confirmLabel: `Delete ${totalCount} Numbers`,
      confirmColor: RED,
      confirmColorDark: RED_DARK,
      onConfirm: () => {
        const ids = (group.items.length > 0 && group.items[0]?.ids !== undefined)
          ? group.items[0].ids
  ...
  const handleBulkEditSubmit = () => {
    const updateData = {};
    if (bulkEditDialog.limit !== "") updateData.limit = parseInt(bulkEditDialog.limit);
    if (bulkEditDialog.rdp_id !== "") updateData.rdp_id = bulkEditDialog.rdp_id;
    if (bulkEditDialog.password_formatter_ids.length > 0) {
      updateData.password_formatters = bulkEditDialog.password_formatter_ids;
    }
  ...
  return (
    <Box>
      ...
      <Box
        ...
        flexWrap="wrap"
        gap={2}
        mb={globalSelectedRows.length > 0 ? 1.5 : 3}
      >
        <Box>
          <Typography
            sx={{
          ...
          <Typography
            variant="caption"
            sx={{ fontSize: "0.75rem", color: alpha(TEXT, 0.55) }}
          >
      ...
      {globalSelectedRows.length > 0 && (
        ...
        >
          ...
          <Typography sx={{ fontSize: "0.8rem", color: BLUE, fontWeight: 600 }}>
            {globalSelectedRows.length} number
            {globalSelectedRows.length !== 1 ? "s" : ""} selected for global
            bulk operations
          </Typography>
          {Object.entries(statusSelectedCounts).map(([status, count]) =>
            count > 0 ? (
              <Chip
                key={status}
                size="small"
                icon={React.createElement(getStatusIcon(status), {
                  sx: { fontSize: "0.65rem !important" },
                })}
                label={`${count} ${getStatusLabel(status).toLowerCase()}`}
                sx={{
                  height: 20,
                  borderRadius: "4px",
                  fontSize: "0.7rem",
                  ...
                  backgroundColor: alpha(STATUS_COLORS[status].color, 0.1),
                  color: STATUS_COLORS[status].color,
                  border: `1px solid ${alpha(STATUS_COLORS[status].color, 0.3)}`,
                  "& .MuiChip-label": { px: 0.7 },
                  "& .MuiChip-icon": { ml: 0.5 },
                }}
              />
          ...
          <Box display="flex" gap={1}>
            <Button
              ...
              variant="contained"
              startIcon={<EditIcon sx={{ fontSize: "0.8rem" }} />}
              onClick={() => handleBulkEditClick(globalSelectedRows)}
              sx={{
                fontSize: "0.72rem",
                py: 0.3,
                px: 1,
            ...
            <Button
              size="small"
              variant="contained"
              startIcon={<DeleteSweepIcon sx={{ fontSize: "0.8rem" }} />}
      ...
      <Box mb={2.5} display="flex" gap={1.5} alignItems="center">
        <StyledTextField
          ...
          InputProps={{
            startAdornment: (
              <SearchIcon
                sx={{ mr: 1, fontSize: "0.9rem", color: alpha(TEXT, 0.4) }}
              />
            ),
          }}
          size="small"
        ...
        <Table size="medium">
          <TableHead>
            ...
            >
              ...
              <TableCell
                colSpan={3}
                sx={{ borderBottom: `2px solid ${alpha(BLUE, 0.5)}` }}
              />
            </TableRow>
          </TableHead>
          <TableBody>
            {!isLoading && filteredNumbers.length === 0 ? (
              ...
              paginatedGroups.map((group, idx) => {
                ...
                for (let i = 0; i < absoluteGroupIdx; i++) {
                  const items = groupedNumbers[i].items;
                  const groupSize = (items.length > 0 && items[0]?.count !== undefined)
                    ? items[0].count
                    : items.length;
      ...
      >
        ...
        >
          Bulk Edit {bulkEditDialog.ids.length} Numbers
        </DialogTitle>
        <DialogContent sx={{ px: 3, py: 2.5 }}>
          <DialogContentText sx={{ mb: 3, fontSize: "0.85rem", color: TEXT }}>
            Enter new values for the fields you want to update. Fields left blank
          ...
          <Grid container spacing={2.5} sx={{ mt: 0.5 }}>
            ...
            <Grid size={{ xs: 12 }}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  multiple
                  value={bulkEditDialog.password_formatter_ids}
                  onChange={handleBulkFormatterChange}
                  input={<OutlinedInput label="Password Formatters" />}
                  renderValue={(selected) => (
        ...
        >
          ...
          <GradientButton
            ...
            sx={{ fontSize: "0.82rem", px: 2, minWidth: 120 }}
          >
            Update {bulkEditDialog.ids.length} Numbers
          </GradientButton>
        </DialogActions>
      </Dialog>
